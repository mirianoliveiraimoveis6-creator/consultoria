...
function atualizarOpcoesBusca() {
    ...
    const imoveisVenda = todosOsImoveis.filter(imovel => {
    ...
    const tipos = imoveisVenda.map(imovel => imovel.Tipo);

    const bairros = imoveisVenda.map(imovel => imovel.Bairro);
...
function mostrarImoveis(imoveis) {
    ...
    tituloResultados.textContent =
        imoveis.length === 1
            ? '1 imóvel à venda'
            : `${imoveis.length} imóveis à venda`;
