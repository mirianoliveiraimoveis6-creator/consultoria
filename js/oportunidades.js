...
function atualizarOpcoesBusca() {
    ...
    const imoveisOportunidade = todosOsImoveis.filter(imovel => {
    ...
    const tipos = imoveisOportunidade.map(imovel => imovel.Tipo);

    const bairros = imoveisOportunidade.map(imovel => imovel.Bairro);
...
function mostrarImoveis(imoveis) {
    ...
    tituloResultados.textContent =
        imoveis.length === 1
            ...
            : `${imoveis.length} oportunidades comercial`;
...
function limparTodosOsFiltros() {
    ...
    mostrarImoveis(imoveisOportunidade);
